...
def coord_trans(bbox, w_pixel, h_pixel, w_amap=7, h_amap=7, mode='a2p'):
  ...
  resized_bbox = resized_bbox.view(bbox.shape[0], -1, bbox.shape[-1])  # CHANGED FROM .view() TO .reshape()
  ...
  if mode == 'p2a':
    ...
    resized_bbox[:, :, [0, 2]] /= width_ratio.view(-1, 1, 1)    # x_min, x_max
    resized_bbox[:, :, [1, 3]] /= height_ratio.view(-1, 1, 1)   # y_min, y_max
  else:
    # activation to pixel
    width_ratio = w_pixel * 1. / w_amap
    height_ratio = h_pixel * 1. / h_amap
...
def ReferenceOnActivatedAnchors(anchors, bboxes, grid, iou_mat, pos_thresh=0.7, neg_thresh=0.3, method='FasterRCNN'):
  ...
  if method == 'FasterRCNN':
    ...
    activated_anc_ind = torch.nonzero(activated_anc_mask.view(-1)).squeeze(-1)
    GT_conf_scores = iou_mat.view(-1)[activated_anc_ind]     # CHANGED .view() to .reshape()
  ...
  activated_anc_coord = anchors.view(-1, 4)[activated_anc_ind]   # CHANGED: .view() to .reshape()
  ...
  negative_anc_coord = anchors.view(-1, 4)[negative_anc_ind.view(-1)]   # CHANGED: .view() to .reshape()
